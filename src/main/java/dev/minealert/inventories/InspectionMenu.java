...
public class InspectionMenu extends AbstractMenu {
    ...
    public InspectionMenu(Player inspected) {
        ...
        setItem(48, builder.itemType(Material.CLOCK).itemAmount(1).itemName("&7Interval").itemLore(List.of("&7Interval: &a" + MineAlertRegistries.getInterval())).build(),
                player -> player.getOpenInventory().close());
    ...
    private String getMapData(int element) {
        ...
        final WeakList<Class<? extends AbstractAlertModule>> tempList = MineDataUtils.getModuleAlertList();
        AbstractAlertModule.getModule(tempList.getElement(element)).ifPresent(fetcher -> mappedAmount.set(fetcher.getMineMap().get(inspected.getName())));
    ...
    private String getDatabaseData(String resultID) {
        ...
        sqlUtils.executeQuery("SELECT * FROM MINEDATA WHERE UUID =?", ps -> ps.setBinaryStream(1,
    ...
    private synchronized void onClick(Player player, Lang mappedValue, Lang overallValue, int listElement, String resultID) {
        CompletableFuture.runAsync(() -> {
            ...
            if (AbstractAlertModule.getModule(DatabaseFile.class).get().getFileConfiguration().getBoolean("enable")) {
            ...
            Bukkit.getScheduler().runTask(MineAlert.getInstance(),() -> player.getOpenInventory().close());
        });
    }
}
